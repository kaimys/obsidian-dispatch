...
1. Resolve the ticket (spec via `id: $ARGUMENTS` in `wiki/05_Requirements/Tickets`, task in the GitHub Issues of `kaimys/obsidian-dispatch`, via the GitHub MCP or the `gh` CLI) and name the session `$ARGUMENTS - <shortened title>` (e.g. `US00042 - Import Meet transcripts`). Confirm today's date with `date`.
...
4. **Preconditions:** the ticket is in `Review`, or in `In progress` when the review was asked for explicitly. A ticket still in `Refinement` has no code to review; say so and stop.
