...
12. **A failed acceptance criterion sends the card back.** Status → `In progress` in both systems, with a dated as-built entry naming the findings that failed it. A review whose findings fail no criterion leaves the card in `Review` and touches nothing else — `open_tests` is the test plan's counter, not the review's, and the review ticks no test-plan items.
