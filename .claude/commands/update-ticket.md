...
1. Find the spec (grep `id: $ARGUMENTS` in `wiki/05_Requirements/Tickets`) and the task in the GitHub Issues of `kaimys/obsidian-dispatch`, via the GitHub MCP or the `gh` CLI. Stop and report if either is missing.
