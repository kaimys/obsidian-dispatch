...
1. Find the spec: grep `id: $ARGUMENTS` in `docs/wiki/02_Requirements/Tickets`. Find the matching task in the GitHub Issues of `kaimys/obsidian-dispatch`, via the GitHub MCP or the `gh` CLI. If either side is missing, stop and report it — don't refine a one-sided ticket.
...
7. **Refinement happens inside `Backlog`** — this board has no separate refinement column, so the gate is the counter, not the column. Leave the status alone. If the ticket is already in `In progress` or beyond, stop and say so instead of dragging it back behind code that already depends on it.
...
11. **Refinement ends when the user says so**, not when the questions run out. Then: acceptance criteria final and testable, `open_questions: 0`, `size:` and `version_target:` set. The ticket stays in `Backlog` — `open_questions: 0` is what marks it buildable, and `/develop` is what moves it.
