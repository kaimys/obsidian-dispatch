...
11. **Refinement ends when the user says so**, not when the questions run out. Then: acceptance criteria final and testable, `open_questions: 0`, `size:` and `version_target:` set. The ticket stays in `Refinement` — `open_questions: 0` is what marks it buildable, and `/develop` is what moves it.
