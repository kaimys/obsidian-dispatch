...
12. **A failed acceptance criterion sends the card back.** Status → `<<S_DEV>>` in both systems, with a dated as-built entry naming the findings that failed it. A review whose findings fail no criterion leaves the card in `<<S_REVIEW>>` and touches nothing else beyond `open_findings` — `open_tests` is the test plan's counter, not the review's, and the review ticks no test-plan items.
