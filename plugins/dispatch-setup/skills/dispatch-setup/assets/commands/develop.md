...
11. When code-complete and green, finish with **`/test-plan $ARGUMENTS`** — it writes the manual plan, sets `open_tests`, stamps the freeze and performs the move to `<<S_REVIEW>>`. Do not make that move by hand; the gate belongs to that command.
