...
7. **Action items go in the allowlisted section as `- [ ]` checkbox lines**, with an owner as a bold line above them or an inline `**Name:**` prefix. **The Todos tab parses exactly this format** — a prose paragraph of "Kai will look into X" is invisible to the board. Set `open_items:` to the number of unchecked items.
