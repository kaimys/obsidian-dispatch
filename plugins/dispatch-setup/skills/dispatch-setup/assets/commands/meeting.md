...
5. **Read the transcript from `<<WIKI>>/<<SOURCES>>` together with the thread** in `discussion:`. The thread usually carries the context the transcript lacks — links, numbers, what people meant. Never edit the transcript; it is a raw source, and a wrong interpretation must always be redoable from it.
   ...
   **Not there yet? Ask for it, and wait.** The transcript arrives by hand: whoever ran the meeting exports it — for a Google Meet recording with Gemini notes, open the document in Google Docs and use *File → Download → Markdown* for **both tabs** — and drops it in `<<SOURCES>>` named `YYYY-MM-DD - <origin> - <title>`. Stop and report rather than writing a report from the agenda; a meeting nobody transcribed has no report.

   *Optional, for teams who do this weekly:* Dispatch ships a script that imports a Meet meeting's Gemini document automatically, so the export step disappears. It needs a Google OAuth client of your own and it is **not** part of the plugin install — see the [`google` block in installation.md](https://github.com/kaimys/obsidian-dispatch/blob/main/docs/installation.md#the-google-block--optional-meet-transcript-import). Worth it for a recurring series; not worth it for one meeting.
...
7. **Action items go in the allowlisted section as `- [ ]` checkbox lines**, with an owner as a bold line above them or an inline `**Name:**` prefix. **The Todos tab parses exactly this format** — a prose paragraph of "Alex will look into X" is invisible to the board. Set `open_actions:` to the number of unchecked items.
