...
- **No wiki/vault structure yet?** Offer to scaffold one before anything else — Dispatch works best on top of an agent-friendly wiki (inspiration: [Karpathy's wiki pattern](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)). Suggested structure (adapt names/domains to the project, numbered for stable ordering):
  ...
  - `01_Product/` (vision, definitions, reports) · `02_Requirements/User-Stories/` (the ticket folder Dispatch will point at) · `03_UX/` · `04_<Domain>/` (the project's subject matter) · `05_Engineering/` (incl. `Decisions/` for ADRs) · `06_Delivery-and-QA/Releases/` (release notes with `version`/`date` frontmatter — feeds the Release Plan tab) · `07_Meetings/` (feeds the Meetings tab) · `08_Discovery/` · `09_Templates/` · `11_Inbox/` (unsorted) · `12_Archive/`
  - `10_Sources/` for **raw, immutable** artifacts (transcripts, feedback, regulations, exports, screenshots): agents read them and write *new* wiki pages — never edit or delete a source, so a wrong interpretation can always be redone from the artifact. Filename `YYYY-MM-DD - <origin> - <title>`, every ingest logged in `log.md` with the pages it changed.
  ...
  - The full worked example — tree, per-folder ownership, sources rules, frontmatter table, precedence, and a four-folder starting point — is [`docs/example-wiki.md`](https://github.com/kaimys/obsidian-dispatch/blob/main/docs/example-wiki.md) in the Dispatch repo. Walk the user through it instead of re-deriving the structure in the interview.
