...
> The one canonical copy of this workflow (ADR-0020). `.claude/commands/test-plan.md` and
> `.codex/skills/test-plan/SKILL.md` are stubs that point here and carry no steps. `<ARGS>` is
> what the caller passed — `<ticket id>`.

Writes the manual test plan and performs the `In progress` → `Review` transition. `<ARGS>` is the ticket id.
...
9. Status → `Review` in both systems. Bump `updated:`.
