...
1. Resolve the ticket (spec via `id: <ARGS>` in `dispatch/wiki/05_Requirements/Tickets`, task in the GitHub Issues of the tracker repository (`tracker.repository` in `dispatch/settings.yaml`), via the GitHub MCP or the `gh` CLI) and name the session `<ARGS> - <shortened title>` (e.g. `US00042 - Import Meet transcripts`). Confirm today's date with `date`.
...
6. **Then the diff, in full.** `git log --oneline develop..HEAD` for the shape of the work, `git diff --stat develop...HEAD` for its extent (against `main` for a patch branch, per step 3), then every new file read whole and every changed file read around its hunks. Tests are part of the diff, not an appendix: read what they assert before judging what the code does.
