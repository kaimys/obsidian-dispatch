...
3. **Duplicate check:** search `wiki/05_Requirements/Tickets` and the GitHub Issues of the tracker repository (`tracker.repository` in `dispatch/settings.yaml`), via the GitHub MCP or the `gh` CLI for the topic. If a matching ticket exists, stop and report it — do not create a near-duplicate.
