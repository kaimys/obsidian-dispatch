...
> The one canonical copy of this workflow (ADR-0020). `.claude/commands/develop.md` and
> `.codex/skills/develop/SKILL.md` are stubs that point here and carry no steps. `<ARGS>` is
...
1. Resolve the ticket (spec via `id: <ARGS>` in `dispatch/wiki/05_Requirements/Tickets`, task in the GitHub Issues of the tracker repository (`tracker.repository` in `dispatch/settings.yaml`), via the GitHub MCP or the `gh` CLI) and name the session `<ARGS> - <shortened title>` (e.g. `US00042 - Import Meet transcripts`).
2. **Pull `develop` first**, then branch off the freshly pulled tip. `develop` carries everything for the next minor version and is where this branch merges back; `main` only moves when a version actually ships, so cutting from it would base the work on the last release rather than on what the version already contains. A stale base forces an avoidable merge later, and on a shared repo it happens within a day. **Two exceptions:** if the ticket's as-built notes name a branch already carrying its code after a re-plan (step 6), continue on that branch and pull the fresh tip into it — the code written before the plan broke is still the code; and a fix destined for a **patch** release branches from `main` and merges back into `main`, bypassing `develop` entirely (`release.md`, "Patch release") — take that path only when the ticket says the fix ships as a patch.
...
6. **Follow the plan — and treat the first contradiction as a stop, not a detour.** The moment reality falsifies a plan step — an API that does not grant what the plan assumed, a data shape that is not there, a path that measures wrong — stop writing code. **Do not try the next idea in this session.** Instead, in this order:
   ...
   - **End the turn**, recommending `/implementation-plan <ARGS>` in a **new session**. The context that produced the wrong plan is the context that will defend it: a long session anchors on the plan it has seen most, and every "this does not work" is weighed against everything before it that said it would. A fresh session reads only the ticket and the code — which is why the record above is written first.
