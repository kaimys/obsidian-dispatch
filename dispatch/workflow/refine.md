...
> The one canonical copy of this workflow (ADR-0020). `.claude/commands/refine.md` and
> `.codex/skills/refine/SKILL.md` are stubs that point here and carry no steps. `<ARGS>` is
...
1. Find the spec: grep `id: <ARGS>` in `wiki/05_Requirements/Tickets`. Find the matching task in the GitHub Issues of `kaimys/obsidian-dispatch`, via the GitHub MCP or the `gh` CLI. If either side is missing, stop and report it — don't refine a one-sided ticket.
