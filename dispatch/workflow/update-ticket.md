...
> The one canonical copy of this workflow (ADR-0020). `.claude/commands/update-ticket.md` and
> `.codex/skills/update-ticket/SKILL.md` are stubs that point here and carry no steps. `<ARGS>` is
...
1. Find the spec (grep `id: <ARGS>` in `dispatch/wiki/05_Requirements/Tickets`) and the task in the GitHub Issues of the tracker repository (`tracker.repository` in `dispatch/settings.yaml`), via the GitHub MCP or the `gh` CLI. Stop and report if either is missing.
...
6. **Tracker comments** on the task, in the GitHub Issues of the tracker repository (`tracker.repository` in `dispatch/settings.yaml`), via the GitHub MCP or the `gh` CLI.
