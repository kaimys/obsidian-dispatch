...
> The one canonical copy of this workflow (ADR-0020). `.claude/commands/implementation-plan.md` and
> `.codex/skills/implementation-plan/SKILL.md` are stubs that point here and carry no steps. `<ARGS>` is
...
11. The ticket stays in `Refinement` once the plan is signed off — on this board the plan is a gate, not a column. Suggest `/develop <ARGS>`.
