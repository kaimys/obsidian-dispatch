...
**Order inside a column.** By default, cards inside a version column sort by workflow progress — furthest-along first — then by Kanban rank, so the column reads as "what's nearly done" from the top.
